...
root_dir = f"/home/ec2-user/quic-efs/user/hansizeng/work/t5_pretrainer/t5_pretrainer/experiments-1M-t5seq-aq/t5seq_aq_encoder_seq2seq_1_lng_knp_mnt_{max_new_token}_dcy_2"
docid_to_smtid_path = os.path.join(root_dir, "aq_smtid/docid_to_smtid.json")
qid_smtid_rerank_path = os.path.join(root_dir, "sub_smtid_32_out/qid_smtid_docids_teacher_score.train.json")
...
old_smtid_to_new_smtid_path = os.path.join(root_dir, "aq_smtid/old_smtid_to_new_smtid.json")


out_dir = os.path.join(root_dir, "mdfy_smtid_sub_smtid_train_decay2")
...
with open(old_smtid_to_new_smtid_path) as fin:
    old_smtid_to_new_smtid = ujson.load(fin)
...
for qid in qid_to_smtid_to_rerank:
    ...
    for smtid, rerank in qid_to_smtid_to_rerank[qid].items():
        ...
        new_smtid = old_smtid_to_new_smtid[smtid]
        if new_smtid != smtid:
            change_count += 1
        total_count += 1
        qid_to_smtid_to_score[qid][new_smtid] = score 

print("change_count = {}, total_count = {}, ratio = {:.3f}".format(change_count, total_count, change_count / total_count))
