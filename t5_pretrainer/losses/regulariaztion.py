...
class L1Diff():
    def __call__(self, input_rep, target_rep):
        assert input_rep.size() == target_rep.size()
        assert input_rep.dim() == 2
        
        return torch.sum(torch.abs(input_rep - target_rep), dim=-1).mean()
